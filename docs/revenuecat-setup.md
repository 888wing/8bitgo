...
1. Create the iOS app with bundle ID `com.eightbitgo.pixelcourier` or replace it in `capacitor.config.ts` and `src/8bitgo/config/appConfig.ts`.
2. Create a non-consumable IAP product.
3. Use product ID `com.eightbitgo.pixelcourier.full_unlock` unless you replace `VITE_8BITGO_FULL_UNLOCK_PRODUCT_ID`.
...
4. Create entitlement `pixel_courier_premium`.
